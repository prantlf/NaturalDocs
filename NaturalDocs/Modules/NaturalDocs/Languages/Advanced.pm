...
#
#   Function: HandleAutoTopics
#
#   A catch-all function for dealing with auto-topics.
#
#   Parameters:
#
#       topicList - A reference to the list of <NaturalDocs::Parser::ParsedTopics> to merge into.
#
sub HandleAutoTopics #(topicList)
    {
    my ($self, $topicList) = @_;

    $self->RepairScope($topicList);
    $self->MergeAutoTopics($topicList);
    $self->MakeAutoGroups($topicList);
    $self->AddScopeDelineators($topicList);
    };
...
#   Parameters:
#
#       topicList - A reference to the list of comment topics.
#
sub RepairScope #(topicList)
    {
    my ($self, $topics) = @_;
    ...
    while (defined $topic)
        ...
        # Finally try to handle the topic, since it has the lowest line number.
            ...
            $topicIndex++;
            $topic = $topics->[$topicIndex];  # Will be undef when past end.
            };
        };
...
sub MergeAutoTopics #(topicList)
    ...
    while ($topicIndex < scalar @$topics && $autoTopicIndex < scalar @$autoTopics)
        {
        my $topic = $topics->[$topicIndex];
...
#   Private Parameters:
#
#       These are used internally for recursion.  Do not set when calling externally.
#
#       startingIndex - The index to start at.
#       endingIndex - The index to end at.
#
...
#       topicList - The list of topics to work on.
#
sub AddScopeDelineators #(topicList)
    {
    my ($self, $topics) = @_;
    ...
    while ($index < scalar @$topics)
        ...
        if ($topic->Scope() ne $currentScope)
            ...
            if ($topic->Type() != ::TOPIC_CLASS() && $topic->Type() != ::TOPIC_CLASS_LIST() &&
                ...
                splice(@$topics, $index, 0, $newTopic);
                $index++;
                };
