...
sub LoadAndPurge
    ...
    if (!NaturalDocs::Settings::RebuildData() && open(SYMBOLTABLEFILEHANDLE, '<' . NaturalDocs::Project::SymbolTableFile()))
        ...
        # There was a bug in prototype detection until 1.12.  Need to regenerate the symbol table.

        if ($version >= NaturalDocs::Version::FromString('1.12') && $version <= NaturalDocs::Settings::AppVersion())
            {  $fileIsOkay = 1;  }
        else
